  ...
  at::parallel_for(
      0, param.numel(), grain_size, [&](int64_t begin, int64_t end) {
        ...
        for (; d < size - (size % Vec::size()); d += Vec::size()) {
          Vec param_vec = Vec::loadu(param_ptr + d); // step 5
          Vec grad_vec =
              Vec::loadu(grad_ptr + d); // step 6
          Vec exp_avg_vec = Vec::loadu(exp_avg_ptr + d) * Vec(beta1) +
              grad_vec * Vec(exp_avg_grad_coefficient); //step 7
          Vec exp_avg_sq_vec = Vec::loadu(exp_avg_sq_ptr + d) * Vec(beta2) +
              grad_vec * grad_vec * Vec(exp_avg_sq_grad_coefficient); // step 8
          exp_avg_vec.store(exp_avg_ptr + d); //step 9
          exp_avg_sq_vec.store(exp_avg_sq_ptr + d); // step 10
          ...
          param_vec = param_vec - Vec(step_size) * exp_avg_vec / denom_vec + param_vec * Vec(weight_decay);
        ...
        for (; d < size; d++) {
          ...
          param_ptr[d] = param_ptr[d] - step_size * exp_avg_ptr[d] / demon_val + param_ptr[d] * weight_decay;
        }
      });
}

template <>
void adamw_fused_step_kernel<at::BFloat16, at::BFloat16>(
    const at::Tensor& param,
    const at::Tensor& exp_avg,
    const at::Tensor& exp_avg_sq,
    const at::Tensor& max_exp_avg_sq,
  ...
  TORCH_CHECK(
      ...
      "adamw_fused_step_kernel: expect param to be at::BFloat16");
  TORCH_CHECK(
      grad.scalar_type() == at::kBFloat16,
      "adamw_fused_step_kernel: expect grad to be at::BFloat16");
  TORCH_CHECK(
      exp_avg.scalar_type() == at::kFloat,
      "adamw_fused_step_kernel: expect exp_avg to be float32");
  TORCH_CHECK(
      exp_avg_sq.scalar_type() == at::kFloat,
      "adamw_fused_step_kernel: expect exp_avg_sq to be float32");
  TORCH_CHECK(
      max_exp_avg_sq.scalar_type() == at::kFloat,
      "adamw_fused_step_kernel: expect max_exp_avg_sq to be float32");
  TORCH_CHECK(
      param2.scalar_type() == at::kBFloat16,
      "adamw_fused_step_kernel: expect param2 to be at::BFloat16");
...
template <>
void adamw_fused_step_kernel<float, at::BFloat16>(
    const at::Tensor& param,
    const at::Tensor& exp_avg,
    const at::Tensor& exp_avg_sq,
    const at::Tensor& max_exp_avg_sq,
  ...
  TORCH_CHECK(
      ...
      "adamw_fused_step_kernel: expect param to be at::Float");
  TORCH_CHECK(
      grad.scalar_type() == at::kBFloat16,
      "adamw_fused_step_kernel: expect grad to be at::BFloat16");
  TORCH_CHECK(
...
void adamw_fused_step_kernel_impl(
  ...
  if (at::ScalarType::Float == grad_dtype) {
    adamw_fused_step_kernel<float, float>(
        param,
        exp_avg,
        exp_avg_sq,
        max_exp_avg_sq,
    ...
    adamw_fused_step_kernel<double, double>(
        param,
        exp_avg,
        exp_avg_sq,
        max_exp_avg_sq,
    ...
    adamw_fused_step_kernel<at::BFloat16, at::BFloat16>(
        param,
        exp_avg,
        exp_avg_sq,
        max_exp_avg_sq,
    ...
    adamw_fused_step_kernel<float, at::BFloat16>(
        param,
        exp_avg,
        exp_avg_sq,
        max_exp_avg_sq,
...
REGISTER_DISPATCH(adamw_fused_step_kernel_stub, &adamw_fused_step_kernel_impl);
