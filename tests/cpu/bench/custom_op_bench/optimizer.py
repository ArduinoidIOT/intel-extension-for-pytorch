...
def non_fused_adamw(param, exp_avg, exp_avg_sq, max_exp_avg_sq, grad, amsgrad, step, beta1, beta2, lr, weight_decay, eps):
    bias_correction1 = 1 - beta1 ** step
    bias_correction2 = 1 - beta2 ** step
    ...
    step_size = lr / bias_correction1
    param.add_(param.mul(weight_decay).addcdiv(exp_avg, denom), alpha=-step_size)
...
def adamw_bench():
    print("Running benchmark for Adam update step")
    fused = torch.ops.torch_ipex.adamw_fused_step
    non_fused = non_fused_adamw
    ...
    for param_size in [1024, 512*1024, 8*1024*1024]:
        ...
        print("For parameter size", param_size)
        run_bench("fused AdamW", fused, param, exp_avg, exp_avg_sq, max_exp_avg_sq, grad, dummy_trail, amsgrad, step, beta1, beta2, learning_rate, weight_decay, eps)
        run_bench("fused split AdamW", fused, param.bfloat16(), exp_avg, exp_avg_sq, max_exp_avg_sq, grad.bfloat16(), trail, amsgrad, step, beta1, beta2, learning_rate, weight_decay, eps)
        run_bench("non fused AdamW", non_fused, param, exp_avg, exp_avg_sq, max_exp_avg_sq, grad, amsgrad, step, beta1, beta2, learning_rate, weight_decay, eps)
